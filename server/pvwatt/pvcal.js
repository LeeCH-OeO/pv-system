async function calculatePVWattsRate(
  ...
  try {
    ...
    const effectiveSolarIrradiance = filteredCloudCover.map(
      (cc) => (100 - cc) / 100
    );

    // // Calculate solar irradiance based on tilt, orientation, and area
    const solarIrradiance = effectiveSolarIrradiance.map(
      (irradiance) =>
        irradiance * (tilt / 100) * (orientation / 100) * (area / 1000)
    );

    // // Calculate daily energy production in kWh
    const dailyEnergyProduction = solarIrradiance.map(
      (irradiance) => irradiance * powerPeak
    );

    // // Adjust energy production based on user-defined system loss
    const adjustedEnergyProduction = dailyEnergyProduction.map(
      (production) => (production * (100 - systemLoss)) / 100
    );

    // // Adjust energy production based on power peak, orientation, tilt, and area
    const adjustedEnergyProductionWithParams = adjustedEnergyProduction.map(
      (production) => production * (powerPeak / 100) * (area / 1000)
    );

    // // Calculate rate of PV/Watts per day
    const ratePVWattsPerDay = adjustedEnergyProductionWithParams.map(
      (production) => production / powerPeak
    );
